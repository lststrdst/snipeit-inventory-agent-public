...
function Start-InitialInventoryRun {
    ...
    Write-InstallLog "Initial inventory started after install/update."
}
...
function Copy-InventoryConfigFiles {
    ...
    foreach ($source in ($configSources | Select-Object -Unique)) {
        ...
        Write-InstallLog "Copied protected config $(Split-Path -Leaf $source)"
    }
...
foreach ($file in $optionalFiles) {
    ...
    if (Test-Path -LiteralPath $source) {
        ...
        Copy-Item -LiteralPath $source -Destination $destination -Force
        Write-InstallLog "Copied optional $file"
    }
}
