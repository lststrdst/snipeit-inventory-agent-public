...
Agent собирает ПК, CPU, RAM, накопители, ОС и текущего интерактивного пользователя. Служебные учётки, включая `ad_*`, `svc_*`, `service_*` и `svc_inventory`, не становятся владельцами. Disabled-пользователь или подтверждённый признак увольнения приводит к возврату актива на склад. Новый корректный пользователь затем может получить тот же актив обычным checkout.
...
Защищённые и служебные логины, включая `snipeit`, администраторов, `krbtgt`, `svc_*`, `service_*` и `svc_inventory`, автоматически не удаляются.
