...
За один запуск обрабатывается не более 10 пользователей. Локальные пользователи Snipe-IT и защищённые логины, включая `snipeit`, `admin`, `administrator`, `krbtgt`, `svc_*`, `service_*` и `svc_inventory`, не удаляются.
