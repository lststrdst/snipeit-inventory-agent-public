...
function Sync-SnipeInventory {
    ...
    $assetNotes = Get-SnipeAssetNotes `
        ...
        -OSBuild $OSBuild `
        -LastBoot $LastBoot `
        -DetectedUsername $ResolvedUsername `
    ...
    if ($SnipeAutoCheckout) {
        $checkoutResult = Set-SnipeAssetCheckout -AssetId $assetId -UserId ([int]$snipeUser.id) -Username $ResolvedUsername -ComputerName $ComputerName -ExistingAsset $asset
    }
    ...
    return [PSCustomObject]@{
        ResultText             = "OK: asset_id=$assetId user=$ResolvedUsername"
        AssetId                = $assetId
...
try {
    ...
    Write-Log "Username detection: raw='$RawDetectedUsername'; canonical='$DetectedUsername'; previous_stored='$PreviousStoredUsername'; previous_canonical='$PreviousUsername'."
